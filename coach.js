...
var firebaseDatosCollection = database.ref().child('coach');

var corf;
var corfu;
firebaseDatosCollection.on('value',function(nocoach){
    corf=0;
    nocoach.forEach(function(firebaseDatosReference){
        var dato = firebaseDatosReference.val();
        console.log(dato);
        corf=corf+1;
    });
    corfu=corf;
    var refHTML =`: `+corfu+`<br>`;
...
function guardar(){
  ...
  var dato1 = {
      clave: corfu, //otra forma es $('#myForm [name="fullname"]').
      nombre: $('#nombre').val(),
  ...
  firebaseDatosCollection.child(corfu).update(dato1);
};

//CREA  'listener' 
firebaseDatosCollection.on('value',function(coach){
...
function borrar(pla)
{
  ...
  let autosRef = this.database.ref('coach/'+pla)
...
function editar(pla)
{
  ...
  database.ref('coach/'+pla).once("value").then(function(snapshot){
